...
// --- TOOLBOX 3 LOGIC (INDEPENDENT INSTANCE) ---
...
const ToolboxManager3 = {
    ...
    renderSpans() {
        const container = document.getElementById('tb3-span-list'); // UPDATED ID
        if (!container) return;
        container.innerHTML = '';
        this.spans.forEach((s, i) => {
            const div = document.createElement('div');
            div.className = 'tb-list-item';
            div.innerHTML = `<span class="label">L${i + 1}</span><input type="number" value="${s}" oninput="ToolboxManager3.spans[${i}] = parseFloat(this.value); ToolboxManager3.calculateBeam()" onchange="ToolboxManager3.spans[${i}] = parseFloat(this.value); ToolboxManager3.calculateBeam()"><span class="unit">mm</span><button class="btn-del" onclick="ToolboxManager3.removeSpan(${i})">×</button>`;
    ...
    renderLoads() {
        const container = document.getElementById('tb3-load-list'); // UPDATED ID
        if (!container) return;
        container.innerHTML = '';
        this.loads.forEach((l, i) => {
            ...
            div.innerHTML = `
                <div class="tb-load-row" style="justify-content:space-between;">
                   <select class="tb-select-sm" onchange="ToolboxManager3.loads[${i}].spanIdx = parseInt(this.value); ToolboxManager3.calculateBeam()">${spanOpt}</select>
                   <select class="tb-select-sm" onchange="ToolboxManager3.changeLoadType(${i}, this.value)">
                        <option value="dist" ${l.type === 'dist' ? 'selected' : ''}>均布荷载</option>
                        <option value="point" ${l.type === 'point' ? 'selected' : ''}>集中荷载</option>
                    </select>
                   <select class="tb-select-sm" onchange="ToolboxManager3.loads[${i}].factor = parseFloat(this.value); ToolboxManager3.calculateBeam()">
                   ...
                   <button onclick="ToolboxManager3.removeLoad(${i})" style="color:#ef4444; border:none; background:none; cursor:pointer;">🗑️</button>
                </div>
                <div class="tb-load-row" style="align-items:center;">
                    <span style="font-size:0.75rem; color:#6b7280; width:50px;">值 Value:</span>
                    <input type="number" class="tb-input-sm" style="flex:1;" value="${l.value}" oninput="ToolboxManager3.loads[${i}].value = parseFloat(this.value); ToolboxManager3.calculateBeam()">
                ...
                <div class="tb-load-row" style="align-items:center;">
                    ...
                    <input type="number" class="tb-input-sm" style="flex:1;" value="${l.dist !== undefined ? l.dist : 0}" oninput="ToolboxManager3.loads[${i}].dist = parseFloat(this.value); ToolboxManager3.calculateBeam()">
                    <span style="font-size:0.75rem; width:30px;">mm</span>
                </div>` : `
                <div class="tb-load-row" style="align-items:center;">
                    <span style="font-size:0.75rem; color:#6b7280; width:50px;">范围:</span>
                    <input type="number" class="tb-input-sm" style="width:60px;" value="${l.range && l.range[0] !== null ? l.range[0] : 0}" oninput="ToolboxManager3.calculateBeam()" placeholder="0">
                    <span style="font-size:0.75rem;">-</span>
                    <input type="number" class="tb-input-sm" style="width:60px;" value="${l.range && l.range[1] !== null ? l.range[1] : this.spans[l.spanIdx]}" oninput="ToolboxManager3.calculateBeam()" placeholder="${this.spans[l.spanIdx]}">
                </div>`}
            `;
            container.appendChild(div);
        });
    ...
    updateSectionType(type) {
        ...
        document.querySelectorAll('.js-sec3-type-toggle button').forEach(b => b.classList.remove('active'));
        document.getElementById(`btn-tb3-sec-type-${type}`).classList.add('active');

        const presetRow = document.getElementById('tb3-channel-preset-row');
        if (presetRow) presetRow.style.display = (type === 'channel') ? 'flex' : 'none';

        const grp4 = document.getElementById('tb3-input-group-4');
        const grpR = document.getElementById('tb3-input-group-r');
        if (grp4) grp4.style.display = (type === 'angle') ? 'none' : 'flex';
        if (grpR) grpR.style.display = (type === 'angle') ? 'flex' : 'none';

        const beamGrpTf = document.getElementById('tb3-beam-grp-tf');
        const beamGrpR = document.getElementById('tb3-beam-grp-r');
    ...
    updateChannelPreset(val) {
        ...
        setVal('tb3-sec-h', p.h); setVal('tb3-sec-b', p.b); setVal('tb3-sec-tw', p.tw); setVal('tb3-sec-tf', p.tf);
        setVal('tb3-beam-sec-h', p.h); setVal('tb3-beam-sec-b', p.b); setVal('tb3-beam-sec-tw', p.tw); setVal('tb3-beam-sec-tf', p.tf);
        this.updateSection();
    },

    updateSection() {
        if (!document.getElementById('tb3-sec-h')) return; // UPDATED ID
        const h = parseFloat(document.getElementById('tb3-sec-h').value); // UPDATED ID
        const b = parseFloat(document.getElementById('tb3-sec-b').value); // UPDATED ID
        const tw = parseFloat(document.getElementById('tb3-sec-tw').value); // UPDATED ID
        const tf = parseFloat(document.getElementById('tb3-sec-tf').value); // UPDATED ID

        let props;
        const canvas = document.getElementById('tb3-sec-canvas'); // UPDATED ID
        canvas.width = canvas.parentElement.clientWidth || 300;
        canvas.height = canvas.parentElement.clientHeight || 450;
        const ctx = canvas.getContext('2d');

        if (this.currentSectionType === 'box') {
            props = SectionProp3.calcBox(h, b, tw, tf);
            SectionProp3.drawBox(ctx, canvas.width, canvas.height, h, b, tw, tf);
        } else if (this.currentSectionType === 'angle') {
            const r = parseFloat(document.getElementById('tb3-sec-r').value) || 0; // UPDATED ID
            // For Angle Iron, use tf as the uniform thickness 't' (since 'web tw' is hidden in UI)
            const t = tf;
            props = SectionProp3.calcAngle(h, b, t, r);
            SectionProp3.drawAngle(ctx, canvas.width, canvas.height, h, b, t, r);
        } else if (this.currentSectionType === 'channel') {
            props = SectionProp3.calcChannel(h, b, tw, tf);
            SectionProp3.drawChannel(ctx, canvas.width, canvas.height, h, b, tw, tf);
        } else {
            props = SectionProp3.calcH(h, b, tw, tf);
            SectionProp3.drawH(ctx, canvas.width, canvas.height, h, b, tw, tf);
        }
        ...
        setText('tb3-res-ix', format(props.Ix)); // UPDATED ID
        setText('tb3-res-w', props.w.toFixed(2)); // UPDATED ID

        setText('tb3-beam-res-ix', format(props.Ix)); // UPDATED ID
        setText('tb3-beam-res-w', props.w.toFixed(2)); // UPDATED ID
        setText('tb3-beam-res-area', format(props.A)); // UPDATED ID
        setText('tb3-beam-res-iy', format(props.Iy)); // UPDATED ID
        setText('tb3-beam-res-wx', format(props.Wx)); // UPDATED ID
        setText('tb3-beam-res-wy', format(props.Wy)); // UPDATED ID

        const grid = document.getElementById('tb3-sec-results'); // UPDATED ID
    ...
    calculateBeam() {
        if (!document.getElementById('tb3-beam-e')) return; // UPDATED ID
        const E = parseFloat(document.getElementById('tb3-beam-e').value); // UPDATED ID
        const h = parseFloat(document.getElementById('tb3-sec-h').value); // UPDATED ID
        const b = parseFloat(document.getElementById('tb3-sec-b').value); // UPDATED ID
        const tw = parseFloat(document.getElementById('tb3-sec-tw').value); // UPDATED ID
        const tf = parseFloat(document.getElementById('tb3-sec-tf').value); // UPDATED ID
        ...
        if (this.currentSectionType === 'box') {
            ...
            const r = parseFloat(document.getElementById('tb3-sec-r').value) || 0; // UPDATED ID
            props = SectionProp3.calcAngle(h, b, tw, r);
        } else if (this.currentSectionType === 'channel') {
            props = SectionProp3.calcChannel(h, b, tw, tf);
        } else {
            props = SectionProp3.calcH(h, b, tw, tf);
        ...
        const swCheck = document.getElementById('tb3-check-self-weight'); // UPDATED ID
        if (swCheck && swCheck.checked) {
            const w_kg = props.w; // kg/m
            const w_std_kN = w_kg * 9.8 / 1000; // Standard SW

            const swFactor = parseFloat(document.getElementById('tb3-sw-factor').value) || 1.2; // UPDATED ID
        ...
        const resULS = BeamSolver3.solve(this.spans, loadsULS, E, props.Ix);
        const resSLS = BeamSolver3.solve(this.spans, loadsSLS, E, props.Ix);
        ...
        const elM = document.getElementById('sum3-m-val'); // UPDATED ID
        ...
        const elV = document.getElementById('sum3-v-val'); // UPDATED ID
        ...
        const elR = document.getElementById('sum3-r-val'); // UPDATED ID
        ...
        const elD = document.getElementById('sum3-def-val'); // UPDATED ID
        if (elD) elD.innerText = dDisp;

        const maxSpan = Math.max(...this.spans);
        const limitVal = maxSpan / 250;
        const elDLimit = document.getElementById('sum3-def-limit'); // UPDATED ID
        if (elDLimit) elDLimit.innerText = `L/250 = ${limitVal.toFixed(1)} mm`;

        const elDStatus = document.getElementById('sum3-def-status'); // UPDATED ID
        if (elDStatus) {
            if (dAbsMax > limitVal) {
        ...
        setChartMax('chart3-max-val-m', mDisp); // UPDATED ID
        setChartMax('chart3-max-val-v', vDisp); // UPDATED ID
        setChartMax('chart3-max-val-r', rDisp); // UPDATED ID
        setChartMax('chart3-max-val-def', dDisp); // UPDATED ID
        ...
        // --- Update External Main View Results (Sync) for Toolbox 3 ---
        const setExt = (id, txt) => { const el = document.getElementById(id); if (el) el.innerText = txt; };
        setExt('ext-tb3-m', mDisp + ' kN·m'); // UPDATED ID
        setExt('ext-tb3-v', vDisp + ' kN'); // UPDATED ID
        setExt('ext-tb3-r', rDisp + ' kN'); // UPDATED ID
        setExt('ext-tb3-def', dDisp + ' mm');
        if (props && props.Wx_Top) setExt('ext-tb3-wx-t', (props.Wx_Top).toLocaleString('en-US', { maximumFractionDigits: 0 }) + ' mm³');
        if (props && props.Wx_Bot) setExt('ext-tb3-wx-b', (props.Wx_Bot).toLocaleString('en-US', { maximumFractionDigits: 0 }) + ' mm³');
        if (props && props.Wy_Left) setExt('ext-tb3-wy-l', (props.Wy_Left).toLocaleString('en-US', { maximumFractionDigits: 0 }) + ' mm³');
        if (props && props.Wy_Right) setExt('ext-tb3-wy-r', (props.Wy_Right).toLocaleString('en-US', { maximumFractionDigits: 0 }) + ' mm³');
        setExt('ext-tb3-status', 'Calculated');

        const extCont = document.getElementById('ext-res-tb3'); // UPDATED ID
        if (extCont) {
    ...
    toggleSelfWeight() {
        const check = document.getElementById('tb3-check-self-weight'); // UPDATED ID
        const detail = document.getElementById('tb3-sw-detail'); // UPDATED ID
    ...
    showDeflectionProcess() {
        ...
        <div style="font-family:'Inter', sans-serif; color:#374151;">
            ...
            <div style="margin-top:20px; text-align:right;">
                 <button onclick="document.getElementById('tb3-def-process-modal').remove()" style="padding:8px 20px; background:#e5e7eb; border:none; border-radius:6px; cursor:pointer; font-weight:600; color:#374151;">Close</button>
            </div>
        </div>
        `;

        // Create Modal Overlay
        const modalId = 'tb3-def-process-modal';
        ...
        modalDiv.innerHTML = `
            <div style="background:white; border-radius:12px; width:90%; max-width:600px; max-height:90vh; overflow-y:auto; box-shadow:0 20px 25px -5px rgba(0,0,0,0.1); padding:0;">
                <div style="padding:20px 25px; border-bottom:1px solid #f3f4f6; display:flex; justify-content:space-between; align-items:center;">
                     <h3 style="margin:0; font-size:1.1rem; color:#111827;">Calculation Process (Toolbox 3)</h3>
    ...
    bindRealTime() {
        const elE = document.getElementById('tb3-beam-e'); // UPDATED ID
    ...
    drawCharts(res) {
        ...
        draw('chart3-loads', '', '#555', 'Load Schematic'); // UPDATED ID
        draw('chart3-reaction', 'r', '#2563eb', 'Reaction (R)'); // UPDATED ID
        draw('chart3-moment', 'm', '#ef4444', 'Moment (kN·m)', 1 / 1000000, true); // UPDATED ID
        draw('chart3-shear', 'v_shear', '#10b981', 'Shear (kN)', 1 / 1000); // UPDATED ID
        draw('chart3-deflection', 'v', '#f59e0b', 'Deflection (mm)', 1); // UPDATED ID
    },

    getData() {
        return {
            sectionType: this.currentSectionType,
            secH: document.getElementById('tb3-sec-h').value,
            secB: document.getElementById('tb3-sec-b').value,
            secTw: document.getElementById('tb3-sec-tw').value,
            secTf: document.getElementById('tb3-sec-tf').value,
            secR: document.getElementById('tb3-sec-r').value,
            beamSecH: document.getElementById('tb3-beam-sec-h').value,
            beamSecB: document.getElementById('tb3-beam-sec-b').value,
            beamSecTw: document.getElementById('tb3-beam-sec-tw').value,
            beamSecTf: document.getElementById('tb3-beam-sec-tf').value,
            beamSecR: document.getElementById('tb3-beam-sec-r').value,
            spans: this.spans,
            loads: this.loads,
            beamE: document.getElementById('tb3-beam-e').value,
            checkSelfWeight: document.getElementById('tb3-check-self-weight').checked,
            swFactor: document.getElementById('tb3-sw-factor').value
        };
    },
    ...
    setData(data) {
        ...
        if (data.secH) document.getElementById('tb3-sec-h').value = data.secH;
        if (data.secB) document.getElementById('tb3-sec-b').value = data.secB;
        if (data.secTw) document.getElementById('tb3-sec-tw').value = data.secTw;
        if (data.secTf) document.getElementById('tb3-sec-tf').value = data.secTf;
        if (data.secR) document.getElementById('tb3-sec-r').value = data.secR;

        if (data.beamSecH) document.getElementById('tb3-beam-sec-h').value = data.beamSecH;
        if (data.beamSecB) document.getElementById('tb3-beam-sec-b').value = data.beamSecB;
        if (data.beamSecTw) document.getElementById('tb3-beam-sec-tw').value = data.beamSecTw;
        if (data.beamSecTf) document.getElementById('tb3-beam-sec-tf').value = data.beamSecTf;
        if (data.beamSecR) document.getElementById('tb3-beam-sec-r').value = data.beamSecR;

        if (data.spans) this.spans = data.spans;
        if (data.loads) this.loads = data.loads;
        if (data.beamE) document.getElementById('tb3-beam-e').value = data.beamE;

        if (data.checkSelfWeight !== undefined) {
            document.getElementById('tb3-check-self-weight').checked = data.checkSelfWeight;
            this.toggleSelfWeight();
        }
        if (data.swFactor) document.getElementById('tb3-sw-factor').value = data.swFactor;

        this.renderSpans();
        this.renderLoads();

        ['h', 'b', 'tw', 'tf', 'r'].forEach(key => {
            const val = document.getElementById(`tb3-sec-${key}`).value;
            document.querySelectorAll(`.js-sec3-${key}`).forEach(el => el.value = val);
        });
...
let activeToolboxId3 = 3;

window.showToolbox3 = (id) => {
    activeToolboxId3 = id;
    document.getElementById('app-main-view').style.display = 'none';
    const tb = document.getElementById('app-toolbox-view-3'); // UPDATED ID
    tb.classList.remove('hidden');
    tb.style.display = 'block';

    document.getElementById('toolbox3-title-text').innerText = `Web-MSteel 结构工具箱 3 (Toolbox 3)`;
    document.title = `StoneCalc - Toolbox 3`;

    setTimeout(() => {
        ToolboxManager3.updateSection();
        window.dispatchEvent(new Event('resize'));
    }, 50);
};

window.closeToolbox3 = () => {
    document.getElementById('app-toolbox-view-3').style.display = 'none'; // UPDATED ID
    document.getElementById('app-toolbox-view-3').classList.add('hidden'); // UPDATED ID
    document.getElementById('app-main-view').style.display = 'flex';
    document.title = 'StoneCalc Pro - Advanced Stone Cladding Calculation';
};

window.tb3AddSpan = () => ToolboxManager3.addSpan();
window.tb3RemoveSpan = (i) => ToolboxManager3.removeSpan(i);
window.tb3AddLoad = () => ToolboxManager3.addLoad();
window.tb3RemoveLoad = (i) => ToolboxManager3.removeLoad(i);
window.tb3CalculateBeam = () => ToolboxManager3.calculateBeam();

window.switchToolboxTab3 = (tab) => {
    document.querySelectorAll('.tab-link-3').forEach(btn => btn.classList.remove('active')); // UPDATED CLASS
    document.querySelectorAll('.tb3-tab-content').forEach(el => el.classList.add('hidden')); // UPDATED CLASS

    if (tab === 'section') {
        document.getElementById('tab-btn-sec-3').classList.add('active'); // UPDATED ID
        document.getElementById('tb3-tab-section').classList.remove('hidden'); // UPDATED ID
        ToolboxManager3.updateSection();
    } else {
        document.getElementById('tab-btn-beam-3').classList.add('active'); // UPDATED ID
        document.getElementById('tb3-tab-beam').classList.remove('hidden'); // UPDATED ID
        setTimeout(() => {
            ToolboxManager3.calculateBeam();
            window.dispatchEvent(new Event('resize'));
        }, 50);
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ToolboxManager3.renderSpans();
    ToolboxManager3.renderLoads();
    ToolboxManager3.bindRealTime();

    const bindSync = (key) => {
        const els = document.querySelectorAll(`.js-sec3-${key}`); // UPDATED CLASS
        els.forEach(el => {
            el.addEventListener('input', (e) => {
                const val = e.target.value;
                document.querySelectorAll(`.js-sec3-${key}`).forEach(other => { // UPDATED CLASS
...
window.ToolboxManager3 = ToolboxManager3;
